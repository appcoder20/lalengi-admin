...
class OrderController extends Controller
{
    ...
    {
        $order = Order::with(['delivery_man', 'store'])->where('id', $request->order_id)->first();
        $dm = $order->delivery_man;
        $store = $order->store;
        $user = 'store';
        if($order->order_type!='parcel'){

        if (in_array($order->order_status, ['handover', 'picked_up']))
        {
            $customer_details = json_decode($order['delivery_address'],true);
            $store = [
                'latitude' => $customer_details['latitude'],
                'longitude' => $customer_details['longitude'],
            ];
            $user = 'customer';
        }

        $deliverymanCoords = $dm->last_location ? ['lat' => $dm->last_location['latitude'], 'lng' => $dm->last_location['longitude']] : ['lat' => 0, 'lng' => 0];

        $storeCoords = $store ? ['lat' => $store['latitude'], 'lng' => $store['longitude']] : ['lat' =>0, 'lng' => 0];
    }
    else{
          ...
          $receiver_details = $order->receiver_details;
             ...
             if (in_array($order->order_status, ['picked_up']))
             {
                $user = 'receiver';
                $storeCoords = ['lat' =>  $receiver_details['latitude'], 'lng' =>  $receiver_details['longitude'] ];
             }
             else{
            
                $user = 'customer';
                $storeCoords = ['lat' =>  $delivery_address['latitude'], 'lng' =>  $delivery_address['longitude'] ];
             }


    }
        return response()->json(['deliveryman' => $deliverymanCoords, 'store' => $storeCoords, 'user' => $user]);
    }
}
