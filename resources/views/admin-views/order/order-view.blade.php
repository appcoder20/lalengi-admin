        ...
        <div class="row flex-xl-nowrap" id="printableArea">
            ...
            <div class="col-lg-4 order-print-area-right">
                ...
                @endif
                      ...
                      <!-- track order -->
                      @if(in_array($order->order_status, ['processing', 'handover', 'picked_up', 'accepted', 'confirmed']) && !is_null($order->delivery_man_id))
                        <div class="card mb-2 mt-2">
                            <div class="card-header border-0 text-center pb-0">
                                <h4 class="m-0">{{ translate('messages.track_order') }} </h4>
                            </div>
                            <div class="card-body pt-2">
                                <div id="track_map" style="height: 500px;"></div>
                            </div>
                        </div>
                   @endif
...
@push('script_2')
  ...
  // Track Order Map JS
  let dm_id = '{{ $order->delivery_man_id }}';
            let store_id = '{{ $order->store_id }}';
            let order_id = '{{ $order->id }}';
            $(document).ready(function() {
            let map, deliverymanMarker, storeMarker, infoWindow, directionsRenderer;

            const initializeTrackMap = () => {
            ...
            const createMarker = (lat, lng, iconUrl) => {
            ...
            const updateDirections = async () => {
                try {
                    const url = `{{ route('admin.order.get-coordinates') }}?order_id=${order_id}`;
                    const response = await fetch(url);
                    const data = await response.json();
                    console.log(data);

                    const deliverymanLat = parseFloat(data.deliveryman.lat);
                    const deliverymanLng = parseFloat(data.deliveryman.lng);
                    const storeLat = parseFloat(data.store.lat);
                    const storeLng = parseFloat(data.store.lng);
                    // const userMarker = data.user === 'store' ? "{{ asset('public/assets/admin/img/restaurant_map.png') }}" : "{{ asset('public/assets/admin/img/customer_location.png') }}";

                    const userMarker =
                                     data.user === 'store'
                                       ? "{{ asset('public/assets/admin/img/restaurant_map.png') }}"
                                       : data.user === 'customer'
                                       ? "{{ asset('public/assets/admin/img/customer_location.png') }}"
                                       : data.user === 'reciver'
                                       ? "{{ asset('public/assets/admin/img/delivery_person.png') }}"
                                       : "{{ asset('public/assets/admin/img/receiver_map_icon.png') }}";


                    // Remove old markers
                    if (deliverymanMarker) {
                        deliverymanMarker.setMap(null);
                    }
                    if (storeMarker) {
                    ...
                    // Create new markers
                    deliverymanMarker = createMarker(deliverymanLat, deliverymanLng, "{{ asset('public/assets/admin/img/delivery_boy_map.png') }}");
                    storeMarker = createMarker(storeLat, storeLng, userMarker);

                    // Calculate and display new route
                    calculateAndDisplayRoute();

                    setTimeout(updateDirections, 500);
                } catch (error) {
            ...
            const calculateAndDisplayRoute = () => {
                ...
                directionsService.route({
                    origin: deliverymanMarker.getPosition(),
                    destination: storeMarker.getPosition(),
                    travelMode: 'DRIVING'
                }, (response, status) => {
                    if (status === 'OK') {
